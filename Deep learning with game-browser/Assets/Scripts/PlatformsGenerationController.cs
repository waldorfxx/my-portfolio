...
public class PlatformsGenerationController : MonoBehaviour
{
    ...
    {
        GeneratePlatform();
        // Debug.Log("SmileGaugeController.currentGauge="+ SmileGaugeController.currentGauge);
        getPlayerPositionX = player.transform.position.x;
        getPlayerPositionY = player.transform.position.y;
    
        if (SmileGaugeController.currentGauge>99&&SmileGaugeController.currentGauge<99.3) {
            GameObject[] normalPlatformList = GameObject.FindGameObjectsWithTag("normalPlatform");
            for (int j = 0; j < normalPlatformList.Length; j++)
                {
                    Debug.Log("SmileGaugeController.currentGauge = "+SmileGaugeController.currentGauge);
                    if(normalPlatformList[j].transform.position.x > getPlayerPositionX+3)
                    {                    
                    Transform temp = normalPlatformList[j].transform;
                    Vector3 vec = temp.position;
                    vec.y = vec.y + 1f;

                    //coin for bonus mode or speed mode

                    GameObject[] coinList = new GameObject[]{coin, coin, coin, coinRed, coin, coin, coinBlue, coin};
                    int index = Random.Range (0, coinList.Length);
                    GameObject temp22 = coinList[index];
                    Instantiate(temp22, vec, temp.rotation);

                    // Instantiate(coinList, vec, temp.rotation);
                    }
                }
        }
